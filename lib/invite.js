...
function handler(log, tMS) {
  return async(req, res) => {
    const srf = req.srf;
    const registrar = req.srf.locals.registrar;
    const ms = req.srf.locals.ms;
    ...
    // outbound call code 29 Jul 2020
      ...
      // const uac = await srf.createUAC(dest, {localSdp: ep.local.sdp});

      // await endpoint.bridge(ep);

    // outbound bridge call
      const callEndpoint = await ms.createEndpoint();
      const callDialog = await srf.createUAC(dest, {localSdp: callEndpoint.local.sdp});
      callEndpoint.modify(callDialog.remote.sdp);

      const { endpoint, dialog } = await ms.connectCaller(req, res);

      callDialog.on('destroy', () => {
      ...
      dialog.on('destroy', () => {
        ...
        dialog.otherlag.de
        callEndpoint.destroy()
      });
      endpoint.bridge(callEndpoint);
...
function makeRtpEngineOpts(req, srcIsUsingSrtp, dstIsUsingSrtp) {
  ...
  return {
    common,
    offer: Object.assign({'sdp': req.body, 'replace': ['origin', 'session-connection'], 'direction': ["pub","pub"]}, common,
      dstIsUsingSrtp ? srtpCharacteristics : rtpCharacteristics),
    answer: Object.assign({'direction': ["pub","pub"]}, common, srcIsUsingSrtp ? srtpCharacteristics : rtpCharacteristics)
  };
}
